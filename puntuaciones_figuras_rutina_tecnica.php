      ...
      <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <div class="card-body">
          ...
          <div class="table-responsive table-sm">
            ...
            $query = "SELECT hibridos_rutina.id, id_rutina, elemento, tipo, texto, nombre, color FROM hibridos_rutina, tipo_hibridos WHERE id_rutina = '$id_inscripcion_figuras' and tipo like 'part' and texto not like '3' and texto=tipo_hibridos.id order by elemento";
                ...
                if(mysqli_num_rows($query_run) > 0){
                  ...
                  while ($row = mysqli_fetch_assoc($query_run)) {
                    ...
                    <tr>
                     <?php
                      ...
                      <th scope="row" style="background-color:<?php echo $row['color'];?>"> <?php echo $row['nombre'].$row['texto'].' F:'.$factor; ?> </th>
                        ...
                        <form action="puntuaciones_figuras_rutina_tecnica_code.php" method="post">
                      <?php
                      $query = "SELECT valor FROM hibridos_rutina WHERE tipo like 'total' and id_rutina=$id_inscripcion_figuras and elemento=".$row['elemento'];
                      $dd = mysqli_fetch_assoc(mysqli_query($connection,$query))['valor'];
                      $query = "SELECT sum(valor) FROM hibridos_rutina WHERE tipo like 'basemark' and id_rutina=$id_inscripcion_figuras and elemento=".$row['elemento'];
                      $basemark = mysqli_fetch_assoc(mysqli_query($connection,$query))['sum(valor)'];
                      $query = "SELECT llevado_BM FROM puntuaciones_elementos WHERE  id_rutina=$id_inscripcion_figuras and elemento=".$row['elemento'];
                    ...
                    while ($row_jueces = mysqli_fetch_assoc($query_run_jueces)) {
                        ...
                        echo  "<th scope='col' $class ><input id='notaE".$row['elemento']."J".$row_jueces['numero_juez']."' name='notaE".$row['elemento']."J".$row_jueces['numero_juez']."' tabindex='".($tab_index+($row_jueces['numero_juez']*9))."' type=number step=0.25 class=form-control value=".$nota." ".$style."></th>";
                        ?>
                    ...
                    $query = "SELECT DISTINCT nota_total, nota_media, nota FROM puntuaciones_elementos WHERE elemento= ".$row['elemento']." and id_rutina=$id_inscripcion_figuras";
                    ...
                    <tr>
                        ...
                        <th colspan="2"><h1>
                            <?php
                    $query = "SELECT nota_final FROM inscripciones_figuras WHERE id=$id_inscripcion_figuras";
                    $nota = mysqli_fetch_assoc(mysqli_query($connection,$query));
...
<!--GUARDAR-->
                        <tr>
                         <td colspan=2>
                          ...
                          <input type="hidden" name="id_inscripcion_figuras" value="<?php echo $id_inscripcion_figuras; ?>">
            ...
            ?>
<script type="text/javascript" src="puntuaciones_figuras_rutina_tecnica.js"></script>
