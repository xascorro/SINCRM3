...
while($fase = mysqli_fetch_array($fases)){
    ...
    //saco numeros de juez del panel tipo 1 (Elementos) y quito si hay un juez de con nombre Media
   	$query = "select paneles.nombre as nombre_panel, numero_juez from panel_jueces, paneles, jueces where id_fase = '".$fase['id']."' and id_panel = paneles.id and (paneles.id_paneles_tipo = 5 or paneles.id_paneles_tipo = 6) and jueces.id = panel_jueces.id_juez and jueces.nombre not like 'Media' order by numero_juez";
   	...
   	while($juez = mysqli_fetch_array($jueces)){
		...
		while($orden = mysqli_fetch_assoc($ordenes)){
			...
			$html .= '<tr><th width="60%">'.'<span>'.$juez['nombre_panel'].'</span></th><th width="40%">'.'<span>NOTA</span>'.'</th></tr>';
			$x=1;
