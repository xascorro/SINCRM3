...
while($fase = mysqli_fetch_array($fases)){
    ...
    if($fase['elementos_coach_card'] <= 0){
        $query = "SELECT numero, nombre, grado_dificultad, corte FROM figuras, fases WHERE fases.id_figura = figuras.id and id_categoria = ".$fase['id_categoria']." and id_competicion = ".$GLOBALS["id_competicion_activa"]." order by orden";
        $numero_fases = mysqli_query($connection,$query)->num_rows;
        $query_run = mysqli_query($connection,$query);
        while($figura = mysqli_fetch_assoc($query_run)){
            $figuras .= '<h6>Corte: '.$figura['corte'].' - Figura '.$figura['numero'].' - '.$figura['nombre'].' GD:'.$figura['grado_dificultad'].'</h6>';
        }
    }
		$html .= '<thead><tr nobr="true"><th width="15%"><h2>'.$nombre_categoria.'</h2></th><th colspan="2" width="85%">'.$figuras.'</th></tr>';
        $html .='<tr><th style="text-align:center"><h4>Orden</h4></th><th width="55%"><h4>Nadadora</h4></th><th width="10%"><h4>Año</h4></th><th width="20%" style="text-align:right"><h4>Club</h4></th></tr>';
        $html .= '</thead><tbody>';
        $figuras = '';
	...
	while($rutina = mysqli_fetch_array($rutinas)){
			...
			$numero = $numero_rutinas.' - ';
        		$html .='<tr nobr="true" style="background-color:'.$rutina_color.'">';
                      ...
                      while($orden_n = mysqli_fetch_assoc($orden_nadadora)){
                        $html .='<th style="text-align:center; background-color:'.$rutina_color.'" width="15%">'.$orden_n['orden'].'</th>';
                    }



        $html .='<td width="55%">'.$rutina['apellidos'].', '.$rutina['nombre'].'</td><td width="10%">'.$rutina['año_nacimiento'].'</td><td width="20%" style="text-align:right;">'.$rutina['nombre_corto'].'</td></tr>';
        ...
        if($fase['elementos_coach_card'] > 0){
            ...
            $html .= '<tr nobr="true" style="background-color:'.$rutina_color.'"><td></td><td colspan="'.($numero_fases-1).'"><h6>';
