				...
				<div class="col-12 col-md-6 mb-3">
					<div class="card border-left-danger shadow h-100 py-2">
						<div class="card-body">
							<div class="row no-gutters align-items-center">
								<div class="col mr-2">
									<div class="font-weight-bold text-danger text-uppercase mb-1">Mi club</div>
									<?php
				...
				<div class="col-12 col-md-6 mb-3">
					<div class="card border-left-success shadow h-100 py-2">
						<div class="card-body">
							...
							<div class="row no-gutters align-items-center">
								<div class="col mr-2">
									<div class="font-weight-bold text-success text-uppercase mb-1">Convalidar Pases de Nivel</div>
									<div class="mb-0 font-weight-bold text-gray-800">Selecciona está opción para convalidar Pases de nivel con Jonrnadas de figuras</div><div class="mb-0 font-weight-bold text-gray-800">Proximamente</div>
								</div>
								<div class="col-auto">
...
//                    $query = 'SELECT * FROM competiciones WHERE activo like "si" and fecha >= now() ORDER BY fecha asc';
                        ...
                        if(mysqli_num_rows($query_run) > 0){
                            while ($row = mysqli_fetch_assoc($query_run)) {
                                echo '<div class="row"><div class="col-12 col-md-6 h5 mb-0 font-weight-bold text-gray-800">'.$row['nombre'].'</div>';
                                echo '<div class="col-12 col-md-3 h6 mb-0 font-weight-bold text-gray-800">'.$row['lugar'].'</div>';
                                echo '<div class="col-12 col-md-2 font-weight-bold text-gray-800">'.
...
//					if($hoy >= $fecha_inicio_inscripcion && $hoy <=$fecha_fin_inscripcion ){
				...
				<?php
                                ...
                                if (file_exists($filename)) {
                                    echo '<div class="col-12 col-md-3"><a href="'.$filename.'" target="_blank"><i class="fa fa-2x fa-file-arrow-down"></i> Normativa</a></div>';
                                }
                                $filename = './docs/'.$row['id'].'-nadadoras.pdf';
                                if (file_exists($filename)) {
                                    echo '<div class="col-12 col-md-3"><a href="'.$filename.'" target="_blank"><i class="fa fa-2x fa-file-arrow-down"></i> Nadadoras</a></div>';
                                }
								$filename = './docs/'.$row['id'].'-inscripciones.pdf';
                                if (file_exists($filename)) {
                                    echo '<div class="col-12 col-md-3"><a href="'.$filename.'" target="_blank"><i class="fa fa-2x fa-file-arrow-down"></i> Inscripciones</a></div>';
                                }
                                $filename = './docs/'.$row['id'].'-orden.pdf';
                                if (file_exists($filename)) {
                                    echo '<div class="col-12 col-md-3"><a href="'.$filename.'" target="_blank"><i class="fa fa-2x fa-file-arrow-down"></i> Orden</a></div>';
                                }
                                $filename = './docs/'.$row['id'].'-resultados.pdf';
                                if (file_exists($filename)) {
                                    echo '<div class="col-12 col-md-3"><a href="'.$filename.'" target="_blank"><i class="fa fa-2x fa-file-arrow-down"></i> Resultados</a></div>';
                                }
                                $filename = './docs/'.$row['id'].'-liga.pdf';
                                if (file_exists($filename)) {
                                    echo '<div class="col-12 col-md-3"><a href="'.$filename.'" target="_blank"><i class="fa fa-2x fa-file-arrow-down"></i> Liga</a></div>';
                                }
                                echo '</div>';
								echo '<div class="border-top border-info my-5"></div>';
							}
                        }else{
                            echo '<div class="row"><div class="col-12 h5 mb-0 font-weight-bold text-gray-800">No existen competiciones programadas</div></div>';
                        }
                        ?>
			</div>
		</div>
                    ...
                    $query = 'select * FROM competiciones WHERE fecha >= (SELECT CURDATE() + 1) ORDER BY fecha asc';
                        ...
                        if(mysqli_num_rows($query_run) > 0){
                            while ($row = mysqli_fetch_assoc($query_run)) {
                                ...
                                date("d M y",strtotime( $row['fecha'] ))
                                .'</div>';
                                echo '<div class="col-1"><a href="'.$row['maps'].'" target=_blank><i class="fa-solid fa-map-location-dot fa-2x text-gray-300"></i></a></div>';
                                echo '</div>';
                            }
                        }else{
                            echo '<div class="row"><div class="col-12 h5 mb-0 font-weight-bold text-gray-800">No existen eventos programados</div></div>';
                        }
                        ?>
				</div>
			</div>
                    ...
                    $query = 'select * FROM competiciones WHERE fecha < (SELECT CURDATE()) ORDER BY fecha desc';
                        ...
                        if(mysqli_num_rows($query_run) > 0){
                            while ($row = mysqli_fetch_assoc($query_run)) {
                                ...
                                echo '<div class="row"><div class="col-12 col-md-6 h5 mb-0 font-weight-bold text-gray-800">'.$row['nombre'].'</div>';
                                echo '<div class="col-12 col-md-3 h6 mb-0 font-weight-bold text-gray-800">'.$row['lugar'].'</div>';
                                echo '<div class="col-9 col-md-2 font-weight-bold text-gray-800">'.
                                date("d M y",strtotime( $row['fecha'] ))
                                .'</div>';
